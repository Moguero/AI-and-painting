...
def rebuild_predictions_with_overlap(
    ...
    n_vertical_patches = (image_tensor.shape[image_height_index] - patch_overlap) // (
        patch_size - patch_overlap
    )
    n_horizontal_patches = (image_tensor.shape[image_width_index] - patch_overlap) // (
        patch_size - patch_overlap
    )
    assert n_vertical_patches * n_horizontal_patches == len(
        main_patch_classes_list
    ), f"The number of patches is not the same : original image should have {n_horizontal_patches*n_vertical_patches} while we have {len(main_patch_classes_list)} "
    ...
    for row_number in range(n_vertical_patches):
        for column_number in range(n_horizontal_patches):
            ...
            patch_tensor = crop_patch_tensor(patch_tensor, patch_overlap)
            if column_number == 0:
                line_rebuilt_tensor = patch_tensor
            else:
                line_rebuilt_tensor = tf.concat([line_rebuilt_tensor, patch_tensor], axis=1)
        # add the right side patches to extend the right side
        right_side_patch_tensor = right_side_patch_classes_list[row_number]
        # todo : first, add cut the right_side_patch_tensor properly
        line_rebuilt_tensor = tf.concat([line_rebuilt_tensor, right_side_patch_tensor], axis=1)
    ...
    ), f"Number of rows is not consistent : got {rebuilt_tensor.shape[rebuilt_height_index]}, expected {int(n_vertical_patches * patch_size - 2 * (patch_overlap / 2))}"
    ...
    ), f"Number of columns is not consistent : got {rebuilt_tensor.shape[rebuilt_width_index]}, expected {int(n_horizontal_patches * patch_size - 2 * (patch_overlap / 2))}"

    rebuilt_tensor = tf.squeeze(rebuilt_tensor)

    logger.info(
        f"\nImage predictions have been successfully built with size {rebuilt_tensor.shape}."
        f"\nOriginal image size : {image_tensor.shape}"
    )
    return rebuilt_tensor
