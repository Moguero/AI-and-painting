...
def stack_image_patch_masks_reformatted(image_patch_path: Path, all_patch_masks_overlap_indices_dict: dict) -> tf.Tensor:
    ...
    problematic_indices_list = ast.literal_eval(all_patch_masks_overlap_indices_dict[str(image_patch_path)])[
        "problematic_indices"]
    if problematic_indices_list:
        categorical_array = stacked_tensor.numpy()
        for pixels_coordinates in problematic_indices_list:
            categorical_array[pixels_coordinates[0], pixels_coordinates[1]] = MAPPING_CLASS_NUMBER["background"]
